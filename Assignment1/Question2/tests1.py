...
class SokobanEncoder:
    ...
    def encode(self):
            ...
            for j in range(0,self.N):
                for b in range(1,self.num_boxes+1) :
                    if i*10+j in self.boxes:
                        self.cnf.append([i*10+j+(1+b)*100])
                    else:
                        self.cnf.append([-(1+b)*100-i*10-j])
        ...
        for t in range(1,self.T):
            for i in range(0,self.N):
                for j in range(0,self.M):
                    for b in range(1,self.num_boxes):
                        B=[]
                        for l in {self.var_box(b,i+1,j,t),self.var_player(i+2,j,t),self.var_player(i+1,j,t+1),-self.var_box(b,i,j,t)}:
                            for m in {self.var_box(b,i-1,j,t),self.var_player(i-2,j,t),self.var_player(i-1,j,t+1),-self.var_box(b,i,j,t)}:
                                for o in {self.var_box(b,i,j+1,t),self.var_player(i,j+2,t),self.var_player(i,j+1,t+1),-self.var_box(b,i,j,t)}:
                                    for p in {self.var_box(b,i,j-1,t),self.var_player(i,j-2,t),self.var_player(i,j-1,t+1),-self.var_box(b,i,j,t)}:
                                        B.append([l,m,p,o])
                        for k in range(0,len(B)):
                            self.cnf.append([-self.var_box(b,i,j,t+1),self.var_box(b,i,j,t)]+B[k])
                        for k in range(0,len(B)):
                            self.cnf.append([-self.var_box(b,i,j,t+1),-self.var_player(b,i,j,t+1)]+B[k])
                ...
                for j in range(0,self.M):
                    for b in range(1,self.num_boxes+1):
                        ...
                        """if (10*i+j in self.walls):
                            self.cnf.append([-self.var_wall(i,j,t+1),-self.var_box(b,i,j,t+1)])"""
        ...
        for t in range (0,self.T+1):              #no two cells can have box simultaneously
            for i in range(0,self.M*10+self.N):
                for j in range(i+1,self.M*10+self.N):
                    for x in range(1,self.num_boxes):
                        for y in range(x+1,self.num_boxes):
                            self.cnf.append([-self.var_box(x,i,j,t),-self.var_box(y,i,j,t)])
        # 5. Goal conditions
        for x in self.goals:
                b=[]
                for b in range(self.num_boxes+1):
                    b.append(self.T*1000+(1+b)*100+x)
                self.cnf.append(b)
        # 6. Other conditions
        return self.cnf
